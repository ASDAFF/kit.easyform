...
foreach ($map as $field) {
    ...
    $arHeaders[] = array("id" => $field->getName(), "content" => (loc::GetMessage("KIT_EASYFORM_FIELD_{$field->getName()}") ?: $field->getTitle()), "sort" => $field->getName(), "default" => (count($arHeaders) < 6));
}
            ...
            case "delete":
                ...
                if (!$tblObj->Delete($propID)) {
                    ...
                    if($ex = $APPLICATION->GetException())
                        $lAdmin->AddGroupError(GetMessage("KIT_EASYFORM_DELETE_ERROR")." [".$ex->GetString()."]", $propID);
                    else
                        $lAdmin->AddGroupError(GetMessage("KIT_EASYFORM_DELETE_ERROR"), $propID);
                }
                $DB->Commit();
                break;
...
$lAdmin->NavText($rsData->GetNavPrint(Loc::GetMessage("KIT_EASYFORM_GET_NAV_PRINT")));
...
while ($arRes = $rsData->Fetch()) {
    ...
    $contentMenu = array(
        array("ICON" => "delete", "TEXT" => Loc::GetMessage("KIT_EASYFORM_DELETE"), "ACTION" => "if(confirm('" . Loc::GetMessage("KIT_EASYFORM_DEL") . "')) " . $lAdmin->ActionDoGroup($arRes["ID"], "delete")),
...
?>
    <form name="form1" method="GET" action="<?= $APPLICATION->GetCurPage() ?>">
        ...
        foreach ($map as $field) {
            ...
            $propsArray["FINDBY[{$field->getName()}]"] = (loc::GetMessage("KIT_EASYFORM_FIELD_{$field->getName()}") ?: $field->getTitle());
        ...
        foreach ($map as $field) :
            ...
            ?>
            <tr>
                <td><?= (loc::GetMessage("KIT_EASYFORM_FIELD_{$field->getName()}") ?: $field->getTitle()) ?>:</td>
                <td>
                    <?
