...
if ($arParams['USE_MODULE_VARNING'] != 'N' && Loader::includeModule("kit.easyform")) {
    if (Option::get("kit.easyform", "SHOW_MESSAGE", "", SITE_ID) == 'Y') {
        $mess = Option::get("kit.easyform", "MESSAGE_TEXT", "", SITE_ID);
        if (strlen($mess) > 0) {
...
if ($request->isPost() && $arParams['FORM_ID'] == $request['FORM_ID']) {
    ...
    if ($arParams["USE_CAPTCHA"]) {
        ...
        if (isset($_REQUEST['g-recaptcha-response'])) {
            ...
            if ($resp->isSuccess()) {
                ...
                $arMessageError[] = Loc::getMessage('KIT_EASYFORM_CAPTCHA_ERROR');
            }
        }
    }
        ...
        if (is_array($field)) {
            ...
            if ($fieldInfo['TYPE'] == 'file') {
                ...
                if (!empty($field['tmp_name'])) {
                    ...
                    $f_id = \CFile::SaveFile($field, 'kit.easyform');
                    if ($f_id)
                        $fieldFiles[] = $f_id;
                } else {
                    $fieldFiles = $field;
    ...
    if ((!empty($arPostFields['EMAIL']) || in_array('EMAIL', $arParams['REQUIRED_FIELDS'])) && !check_email($arPostFields['EMAIL']))
        $arMessageError[] = Loc::getMessage('KIT_EASYFORM_EMAIL_ERROR');
    ...
    if ($emptyForm) {
        $arMessageError[] = Loc::getMessage('KIT_EASYFORM_EMPTY_FORM_ERROR');
    }
    ...
    if (!in_array('NONE', $arParams['REQUIRED_FIELDS'])) {
        foreach ($arParams['REQUIRED_FIELDS'] as $FIELD) {
            ...
            if ((empty($arParams['REQUIRED_FIELDS']) || in_array($FIELD, $arParams['REQUIRED_FIELDS'])) && strlen($arPostFields[$FIELD]) == 0) {
                $arMessageError[] = Loc::getMessage("KIT_EASYFORM_FIELD_ERROR_MESS", array("#FIELD_NAME#" => $message_field));
            }
        }
        unset($FIELD);
    }
    ...
    if (empty($arMessageError) && $arParams['USE_IBLOCK_WRITE'] == 'Y' && intval($arParams['IBLOCK_ID']) > 0 && Loader::IncludeModule("iblock")) {
        ...
        foreach (GetModuleEvents("kit.easyform", "OnBeforeIBlockElementAdd", true) as $arEvent)
        ...
        if ($idElement = $el->Add($arLoadProductArray)) {
            ...
            foreach (GetModuleEvents("kit.easyform", "OnAfterIBlockElementAdd", true) as $arEvent)
    ...
    if (Loader::IncludeModule("kit.easyform") && empty($arMessageError)) {
        $fields = array();
        ...
        if (is_array($arParams['EVENT_MESSAGE_ID']) && !empty($arParams['EVENT_MESSAGE_ID'])) {
            foreach ($arParams['EVENT_MESSAGE_ID'] as $arMessID) {
                if (!CEvent::Send($arParams['EVENT_NAME'], SITE_ID, $arServiceFields, 'N', $arMessID, $file_id)) {
                    $arMessageError[] = Loc::getMessage('KIT_EASYFORM_SEND_ERROR_MESS', array("#MESS_ID#" => $arMessID));
                }
            }
        } else {
            if (!CEvent::Send($arParams['EVENT_NAME'], SITE_ID, $arServiceFields, 'N', "",  $file_id)) {
                $arMessageError[] = Loc::getMessage('KIT_EASYFORM_SEND_ERROR');
            }
        }
    ...
    foreach (GetModuleEvents("kit.easyform", "OnAfterAdditionalMessage", true) as $arEvent)
...
if ($arParams["USE_CAPTCHA"] && !defined("KIT_RE_CAPTCHA_INCLUDE")) {
    $APPLICATION->AddHeadScript('https://www.google.com/recaptcha/api.js');
    define("KIT_RE_CAPTCHA_INCLUDE", "Y");
}

if ($arParams["USE_JQUERY"] && !defined("KIT_JQUERY_INCLUDE")) {
    $APPLICATION->AddHeadScript($componentPath . '/lib/js/jquery-1.12.4.min.js');
    define("KIT_JQUERY_INCLUDE", "Y");
}

if ($arParams["USE_BOOTSRAP_JS"] == 'Y' && !defined("KIT_BOOTSTRAP_JS_INCLUDE")) {
    $APPLICATION->AddHeadScript($componentPath . '/lib/js/bootstrap.min.js');
    define("KIT_BOOTSTRAP_JS_INCLUDE", "Y");
}

if ($arParams["USE_BOOTSRAP_CSS"] == 'Y' && !defined("KIT_BOOTSTRAP_CSS_INCLUDE")) {
    $APPLICATION->SetAdditionalCSS($componentPath . '/lib/css/bootstrap.min.css');
    define("KIT_BOOTSTRAP_CSS_INCLUDE", "Y");
}

if ($arParams['USE_FORMVALIDATION_JS'] == "Y" && $arParams['INCLUDE_BOOTSRAP_JS'] != "N" && !defined("KIT_BOOTSTRAP_VALIDATOR_INCLUDE")) {
    ...
    define("KIT_BOOTSTRAP_VALIDATOR_INCLUDE", "Y");
}

if ($arParams["USE_INPUTMASK_JS"] == 'Y' && !defined("KIT_FORM_INPUTMASK_INCLUDE")) {
    $APPLICATION->AddHeadScript($componentPath . '/lib/js/inputmask.js');
    define("KIT_FORM_INPUTMASK_INCLUDE", "Y");
}

if (!defined("KIT_FORM_JS_INCLUDE")) {
    $APPLICATION->AddHeadScript($componentPath . '/script.js');
    define("KIT_FORM_JS_INCLUDE", "Y");
}
